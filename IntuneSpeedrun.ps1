...
.NOTES
	...
	Adapted from:       Per Larsen / Frank Simorjay / Alex Fields
    Created:            2021.11.14
	Revised:     
    Version:            0.1
    PS Modules required:    AzureADPreview, Microsoft.Graph.Intune, MSAL.ps
    
    NOTE:               Must use Powershell 7 to use MSAL tokens
                        Authentication twice is expected, unable to pass $authtoken to Connect-AzureAD at this time

    Limitations:        Administrative Template PATCH actions are present but not functional
                        ChromeADMX is imported with "placeholder" definition due to problems including XML into JSON
...
<#function Get-AuthToken {
    ...
    # Checking for PS Module prerequisites
    Write-Host "Checking for AzureADPreview module..."
        ...
        if ($null -eq $AadModule) {
            ...
            write-host "Install by running 'Install-Module AzureADPreview -Force -AllowClobber' from an elevated PowerShell prompt" -f Yellow
...
function Get-AuthToken {
    ...
    try {
        ...
        $authResult = Get-MsalToken -ClientId "d1ddf0e4-d672-4dae-b554-9d5bdfd93547" -TenantId $tenant -DeviceCode
        # If the accesstoken is valid then create the authentication header
        if ($authResult) {
            # Creating header for Authorization token
            $authHeader = @{
...
Function New-IntuneDefaultGroups {
    ...
    New-AzureADGroup -DisplayName "Intune - All Users" -MailEnabled $false -MailNickName "IntuneAllUsers" -SecurityEnabled $true 
    New-AzureADGroup -DisplayName "Intune - Autopilot Exempt" -MailEnabled $false -MailNickName "IntuneAPExempt" -SecurityEnabled $true
    New-AzureADMSGroup -DisplayName "Intune - All Devices" `
        -MailEnabled $false `
...
####################################################
Function Add-DefaultAutoPilotProfile {

    <#
    .SYNOPSIS
    This function is used to add a baseline Autopilot profile per current Resultant standards
    .DESCRIPTION
    The function connects to Azure AD to create a baseline Autopilot profile
    .EXAMPLE
    Add-DefaultAutoPilotProfile
    Adds Resultant standard default group structure
    .NOTES
    NAME:Add-DefaultAutoPilotProfile
    #>
    ...
    $AP_resource = "deviceManagement/windowsAutopilotDeploymentProfiles"
    Write-Verbose "Resource: $AP_resource"
...
####################################################
Function Add-AdministrativeTemplate(){

    <#
    .SYNOPSIS
    This function is used to add an device configuration policy using the Graph API REST interface
    .DESCRIPTION
    The function connects to the Graph API Interface and adds an Administrative Template
    .EXAMPLE
    Add-AdministrativeTemplate -JSON $JSON
    Adds a device configuration policy in Intune
    .NOTES
    NAME: Add-AdministrativeTemplate
    #>
    ...
    $graphApiVersion = "Beta"
    $DCP_resource = "deviceManagement/groupPolicyDefinitions"
    Write-Verbose "Resource: $DCP_resource"
    
    try {    
        if($JSON -eq "" -or $null -eq $JSON){    
            write-host "No JSON specified, please specify valid JSON for the Administrative Template..." -f Red    
        } else {    
            Test-JSON -JSON $JSON
            $uri = "https://graph.microsoft.com/$graphApiVersion/$($DCP_resource)"
            Invoke-RestMethod -Uri $uri -Headers $authToken -Method Post -Body $JSON -ContentType "application/json"    
...
####################################################
    $DefaultAutoPilot = @"
    {
        ...
        "id": "dfd5c946-06f3-4f63-a29b-56fc2b600f33",
        "displayName": "Default",
        "description": "",
        "language": "os-default",
        "createdDateTime": "2021-11-19T18:15:04.9164766Z",
        "lastModifiedDateTime": "2021-11-19T18:15:04.9164766Z",
...
####################################################
#Non-Windows device configuration profiles
####################################################
    $MacOSDR = @"
...
####################################################
# Device Endpoint protecion definition     
    $Win10BASICEP = @"
...
####################################################
# Enterprise  Device Restriction definition   
    $Win10DR = @"
    {
        "@odata.type":  "#microsoft.graph.windows10GeneralConfiguration",
        "deviceManagementApplicabilityRuleOsEdition":  null,
...
####################################################
# Enterprise Device Endpoint protecion definition    
    $Win10EP = @"
    {
        "@odata.type":  "#microsoft.graph.windows10EndpointProtectionConfiguration",
        "deviceManagementApplicabilityRuleOsEdition":  null,
...
####################################################
# Windows Update for Business Pilot group Definition   
    $UpdatePilot = @"
    {
...
####################################################
# Windows Update for Business Full group Definition   
    $UpdateBroad = @"
    {
        "@odata.type":  "#microsoft.graph.windowsUpdateForBusinessConfiguration",
        "description":  "",
...
####################################################
    ...
    {
        "@odata.type":  "#microsoft.graph.windows10CustomConfiguration",
        "displayName":  "Default App Associations",
        "description":  null,
        "omaSettings":  [
                            {
                                "@odata.type":  "#microsoft.graph.omaSettingString",
...
####################################################
# Administrative template declarations
####################################################
<#
# Future use
    $OneDriveKFM = @"
    ...
    {
        "definition@odata.bind":  "https://graph.microsoft.com/beta/deviceManagement/groupPolicyDefinitions(\u00271970d0f6-7d81-46ba-9bb1-eb8856b39161\u0027)",
        "enabled":  true
    },
    {
...
####################################################
    ...
    $Answer = Read-Host "Do you want to create the default group set (Recommended)? Type Y or N and press Enter to continue"
    if ($Answer -eq 'y' -or $Answer -eq 'yes') {
        Write-Host "Connecting to AzureAD" -ForegroundColor Yellow
        Import-Module AzureADPreview -UseWindowsPowershell
        Connect-AzureAD -AccountId $user
        Write-Host "Adding default user and device groups" -ForegroundColor Yellow
        New-IntuneDefaultGroups    
        Write-Host 
    } else {
        Write-Host "Default user and device groups will not be created" -ForegroundColor Red
        Write-Host
    }
...
#>
    ...
    $Answer = Read-Host "Do you want to import Default AutoPilot profile (Optional)? Type Y or N and press Enter to continue"
    if ($Answer -eq 'y' -or $Answer -eq 'yes') {    
        Write-Host "Adding Default AutoPilot profile..." -ForegroundColor Yellow    
        Add-DefaultAutoPilotProfile -JSON $DefaultAutoPilot
    
        Write-Host
    } else { 
        Write-Host "Default AutoPilot profile will not be imported" -ForegroundColor Red
        Write-Host 
    }   
...
<#    
    ...
    $Answer = Read-Host "Do you want to import Administrative Templates for Windows 10 (Optional)? Type Y or N and press Enter to continue"
    if ($Answer -eq 'y' -or $Answer -eq 'yes') {
        $tenantguid = Read-Host "Please enter the Azure Tenant GUID"
        Write-Host "Adding Administrative Templates..." -ForegroundColor Yellow    
        Add-AdministrativeTemplate -JSON $OneDriveKFM
        Write-Host
    } else { Write-Host "Administrative Templates will not be imported" -ForegroundColor Red
    Write-Host 
    }       
#>
    ####################################################
    ...
    if ($Answer -eq 'y' -or $Answer -eq 'yes') {
        ...
        Add-DeviceConfigurationPolicy -Json $DefaultApps
        Write-Host 
        ...
        Write-Host "Adding Administrative Templates..." -ForegroundColor Yellow 
        Add-DeviceConfigurationPolicy -Json $ChromeADMX
        Write-Host
